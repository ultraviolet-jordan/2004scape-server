package com.lostcity.session.encode

import com.google.inject.Singleton
import com.lostcity.session.CodecEncoder
import com.lostcity.session.Session
import com.lostcity.session.type.BadSessionResponse
import com.lostcity.session.type.YouAreStandingInAMembersOnlyAreaResponse
import jagex2.io.p1
import java.nio.ByteBuffer

@Singleton
class YouAreStandingInAMembersOnlyAreaEncoder : CodecEncoder<YouAreStandingInAMembersOnlyAreaResponse> {
    override suspend fun encode(session: Session, response: YouAreStandingInAMembersOnlyAreaResponse): ByteBuffer {
        val buffer = ByteBuffer.allocate(1)
        buffer.p1(17)
        return buffer.flip()
    }
}