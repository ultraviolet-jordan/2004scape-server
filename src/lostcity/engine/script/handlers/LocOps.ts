...
const LocOps: CommandHandlers = {
    [ScriptOpcode.LOC_ADD]: state => {
        ...
        const loc: Loc = new Loc(position.level, position.x, position.z, locType.width, locType.length, locType.id, locShape, locAngle);
    ...
    [ScriptOpcode.LOC_CHANGE]: checkedHandler(ActiveLoc, state => {
        ...
        const loc = new Loc(state.activeLoc.level, state.activeLoc.x, state.activeLoc.z, locType.width, locType.length, locType.id, state.activeLoc.shape, state.activeLoc.angle);
        World.addLoc(loc, duration);
    ...
    [ScriptOpcode.LOC_COORD]: checkedHandler(ActiveLoc, state => {
        const {level, x, z} = state.activeLoc;
        state.pushInt(Position.packCoord(level, x, z));
    }),
    ...
    [ScriptOpcode.LOC_NAME]: checkedHandler(ActiveLoc, state => {
        ...
        state.pushString(locType.name ?? 'null');
    }),
