...
const InvOps: CommandHandlers = {
    ...
    [ScriptOpcode.INV_ADD]: checkedHandler(ActivePlayer, state => {
        ...
        const overflow = count - player.invAdd(invType, objId, count);
        if (overflow > 0) {
    ...
    [ScriptOpcode.INV_CLEAR]: checkedHandler(ActivePlayer, state => {
        ...
        state.activePlayer.invClear(invType);
    }),
    ...
    [ScriptOpcode.INV_DEL]: checkedHandler(ActivePlayer, state => {
        ...
        state.activePlayer.invDel(invType, objType, count);
    }),
    ...
    [ScriptOpcode.INV_DELSLOT]: checkedHandler(ActivePlayer, state => {
        ...
        const obj = state.activePlayer.invGetSlot(invType, slot);
        if (!obj) {
            return;
        }

        state.activePlayer.invDelSlot(invType, slot);
    }),
    ...
    [ScriptOpcode.INV_DROPITEM]: checkedHandler(ActivePlayer, state => {
        ...
        const player = state.activePlayer;
        const completed = player.invDel(invType, objType, count);
        if (completed == 0) {
            return;
        }
    ...
    [ScriptOpcode.INV_DROPSLOT]: checkedHandler(ActivePlayer, state => {
        ...
        const objType: ObjType = check(obj.id, ObjTypeValid);
        ...
        const player = state.activePlayer;
        const completed = player.invDel(invType, objType, obj.count, slot);
        if (completed == 0) {
            return;
        }
    ...
    [ScriptOpcode.INV_FREESPACE]: checkedHandler(ActivePlayer, state => {
        ...
        state.pushInt(state.activePlayer.invFreeSpace(invType));
    }),
    ...
    [ScriptOpcode.INV_ITEMSPACE]: checkedHandler(ActivePlayer, state => {
        ...
        if (size < 0 || size > invType.size) {
            throw new Error(`size is out of range: ${size}`);
        }

        state.pushInt(state.activePlayer.invItemSpace(invType, objType, count, size) == 0 ? 1 : 0);
    }),
    ...
    [ScriptOpcode.INV_ITEMSPACE2]: checkedHandler(ActivePlayer, state => {
        ...
        state.pushInt(state.activePlayer.invItemSpace(invType, objType, count, size));
    }),
    ...
    [ScriptOpcode.INV_MOVEFROMSLOT]: checkedHandler(ActivePlayer, state => {
        ...
        const fromType: InvType = check(fromInv, InvTypeValid);
        const toType: InvType = check(toInv, InvTypeValid);

        if (!state.pointerGet(ProtectedActivePlayer[state.intOperand]) && fromType.protect && fromType.scope !== InvType.SCOPE_SHARED) {
            throw new Error(`$inv requires protected access: ${fromType.debugname}`);
        }

        if (!state.pointerGet(ProtectedActivePlayer[state.intOperand]) && toType.protect && fromType.scope !== InvType.SCOPE_SHARED) {
            throw new Error(`$inv requires protected access: ${toType.debugname}`);
        }

        const player = state.activePlayer;
        const { overflow, fromObj } = player.invMoveFromSlot(fromType, toType, fromSlot);
        if (overflow > 0) {
    ...
    [ScriptOpcode.INV_MOVETOSLOT]: checkedHandler(ActivePlayer, state => {
        ...
        state.activePlayer.invMoveToSlot(fromType, toType, fromSlot, toSlot);
    }),
    ...
    [ScriptOpcode.BOTH_MOVEINV]: checkedHandler(ActivePlayer, state => {
        ...
        const fromType: InvType = check(from, InvTypeValid);
        const toType: InvType = check(to, InvTypeValid);
        ...
        if (!state.pointerGet(ProtectedActivePlayer[secondary ? 1 : 0]) && fromType.protect && fromType.scope !== InvType.SCOPE_SHARED) {
            throw new Error(`$from_inv requires protected access: ${fromType.debugname}`);
        }

        if (!state.pointerGet(ProtectedActivePlayer[secondary ? 0 : 1]) && toType.protect && fromType.scope !== InvType.SCOPE_SHARED) {
            throw new Error(`$to_inv requires protected access: ${toType.debugname}`);
        }

        const fromInv = fromPlayer.getInventory(fromType.id);
        const toInv = toPlayer.getInventory(toType.id);
    ...
    [ScriptOpcode.INV_MOVEITEM]: checkedHandler(ActivePlayer, state => {
        ...
        const completed = state.activePlayer.invDel(fromType, objType, count);
        if (completed == 0) {
            return;
        }

        state.activePlayer.invAdd(toType, objType.id, completed);
    }),
    ...
    [ScriptOpcode.INV_MOVEITEM_CERT]: checkedHandler(ActivePlayer, state => {
        ...
        const overflow = count - state.activePlayer.invAdd(toType, finalObj, completed);
        if (overflow > 0) {
    ...
    [ScriptOpcode.INV_MOVEITEM_UNCERT]: checkedHandler(ActivePlayer, state => {
        ...
        if (objType.certtemplate >= 0 && objType.certlink >= 0) {
            state.activePlayer.invAdd(toType, objType.certlink, completed);
        } else {
            state.activePlayer.invAdd(toType, objType.id, completed);
        }
    }),
    ...
    [ScriptOpcode.INV_SETSLOT]: checkedHandler(ActivePlayer, state => {
        ...
        state.activePlayer.invSet(invType, objType.id, count, slot);
    }),
    ...
    [ScriptOpcode.INV_TOTAL]: checkedHandler(ActivePlayer, state => {
        ...
        state.pushInt(state.activePlayer.invTotal(invType, obj));
    }),
    ...
    [ScriptOpcode.INV_TRANSMIT]: checkedHandler(ActivePlayer, state => {
        ...
        state.activePlayer.invListenOnCom(invType, com, state.activePlayer.uid);
    }),
    ...
    [ScriptOpcode.INVOTHER_TRANSMIT]: checkedHandler(ActivePlayer, state => {
        ...
        state.activePlayer.invListenOnCom(invType, com, uid);
    }),
